...
**已知盲区（实现同步）**：共享守卫现对 `crate_use_violations` 与
`layering_violations` 扫描每一行的有效代码，而不是只看 `use` 前缀；因此行内
全限定路径已有合成违规自证，`platform_cfg_violations` 仍是全行 `contains`，两者
不应混为一谈。守卫依然是词法匹配：不展开宏或 `include!` 输入，非规范空格（如
`crate :: infrastructure`）可绕过，且不解析字符串字面量。这些限制在共享测试支持
中显式注释；对需要更强语义的新增规则应走审查或单独设计，而非在此变更中引入 AST
分析框架。
...
本 change 归档时遗留 4 条未勾选任务,现逐一复核;未完成项以下述书面结论关闭(对应 design「复审修正」机制)。

- **7.7 场景命令按模块聚合(降低重复度)**:复核时(scenario-commands.mjs, 2026-09-21)实测 291 → 302 条场景对应 197 条唯一命令,重复度从归档时记录的 1.82× 降至 1.48×。未按「模块聚合到更少命令批次」重做:**以延期关闭**。理由:现命令已细到具体测试名,聚合会失去每条场景到测试名的追溯(该追溯是 validate-scenario-commands.mjs 与 churn ratchet 的基础);重复度持续下降后,进一步聚合的收益低于追溯粒度损失。该项不阻塞发布,列后续治理优化。
- **8.4 真机播放冒烟**:人工验证项,未在归档前完成。**以复核结论关闭**:对应行为已由自动化覆盖播放控制/恢复的 `cargo test` 与真实 Release 产物(`v0.1.0`)的 macOS 启动冒烟(task-1.9 的 cold/hot 单实例、libmpv 加载、tray 注册)部分替代;完整人工手感冒烟留待后续真机会话。
- **8.5 归档前逐条审计确认**:本次归档复核即执行该审计,结论见上;其余任务(7.8 声明「归档目录不再有未勾选任务」)被证伪 —— 本 change 及另外 3 个 archived change 均留有未勾选任务,已在本次复核中逐条处置。
