...
- 单个源文件控制规模：一般不超过 500 行；超过 1000 行的文件必须拆分（产品原型、生成代码与编译产物除外）。
- 按职责与边界拆分，不以行数为唯一依据：一个文件承载一种抽象（一个领域聚合、一个状态机、一类 Repository/Port 实现，或一组字段结构相关的小 fake），不把无关实现堆进单个文件。
- 文件增长至接近上限时应先拆分再扩展；新增代码归入职责匹配的既有文件，避免类别混杂。
- 拆分沿功能边界进行，每个文件保留说明其职责的模块 doc；共享类型和 re-export 位于模块根部，公共入口保持稳定。
- 相关测试就近放在被测模块旁，不集中成巨型测试文件；维护者可一眼看出某个 fake、适配器或状态机的全部实现与测试。
- 单个函数也应保持单一职责：过长函数按可命名步骤拆分，避免无法独立阅读的巨型函数。
