...
系统 MUST 在 Release 中附带桌面端打包进应用的第三方组件的许可证说明。至少包含 macOS、Windows、Linux 三个平台打包目录 `apps/desktop/src-tauri/vendor/libmpv/{macos,windows,linux}/NOTICE.md` 所声明的 libmpv 及关联库的许可证文本或来源,随安装包与校验清单一同上传,使终端用户可获取。
