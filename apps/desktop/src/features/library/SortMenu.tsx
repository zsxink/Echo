...
export interface SortValue<Field extends string> {
...
interface SortMenuProps<Field extends string> {
  readonly sort: SortValue<Field>;
  readonly options: ReadonlyArray<SortOption<Field>>;
  readonly onChange: (sort: SortValue<Field>) => void;
}

/** Shared compact sort menu for songs and collection directories. */
export function SortMenu<Field extends string>({ sort, options, onChange }: SortMenuProps<Field>) {
      ...
      >
        {options.map((option) => (
          <button
            ...
            type="button"
            role="menuitemradio"
            aria-checked={option.value === sort.field}
            className={`sort-option${option.value === sort.field ? " active" : ""}`}
            onClick={() => {
              onChange({ ...sort, field: option.value });
              setOpen(false);
            }}
          >
