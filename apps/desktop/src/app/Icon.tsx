...
const GLYPHS: Record<string, Glyph> = {
  ...
  upload: { nodes: <path d="M12 15V3m-4 4 4-4 4 4M4 15v5h16v-5" /> },
  ...
  // --- transport -----------------------------------------------------------
  // 传输控制用圆角三角形。圆角不是"内切圆角"——那是按切点距 = r/tan(θ/2) 反推
  // 半径，会把窄角（skip 的上下角）啃掉大半；这里是**固定切点距**，沿两边各收
  // 缩 2.2，弧半径由内角自动推出，形状才保得住。圆角会让尖端向内缩约 0.93/24
  // 网格，所以 x 坐标已做光学居中补偿。
  //
  // previous 是 next 的**严格水平镜像**：两者的 signed area 互为相反数
  // (±68.88)，镜像对的圆弧 sweep-flag 也相反（屏幕坐标下正面积 = 顺时针）。
  // 只改其中一条会让圆角往形状外侧凸、把三角啃出缺口。
  play: {
    solid: true,
    nodes: (
      <path d="M4.27 5.8A1.34 1.34 0 0 1 6.23 4.6L18.71 11A1.13 1.13 0 0 1 18.71 13L6.23 19.4A1.34 1.34 0 0 1 4.27 18.2L4.27 5.8Z" />
    ),
  },
  pause: {
    solid: true,
    nodes: (
      <>
        <rect x="5.76" y="3.6" width="5.07" height="16.8" rx="2.2" />
        <rect x="13.17" y="3.6" width="5.07" height="16.8" rx="2.2" />
      </>
    ),
  },
  previous: {
    solid: true,
    nodes: (
      <path d="M11.53 5.8A0.9 0.9 0 0 0 9.99 5.17L4.87 10.43A2.25 2.25 0 0 0 4.87 13.57L9.99 18.83A0.9 0.9 0 0 0 11.53 18.2L11.53 5.8ZM19.73 5.8A0.9 0.9 0 0 0 18.19 5.17L13.07 10.43A2.25 2.25 0 0 0 13.07 13.57L18.19 18.83A0.9 0.9 0 0 0 19.73 18.2L19.73 5.8Z" />
    ),
  },
  next: {
    solid: true,
    nodes: (
      <path d="M4.27 5.8A0.9 0.9 0 0 1 5.81 5.17L10.93 10.43A2.25 2.25 0 0 1 10.93 13.57L5.81 18.83A0.9 0.9 0 0 1 4.27 18.2L4.27 5.8ZM12.47 5.8A0.9 0.9 0 0 1 14.01 5.17L19.13 10.43A2.25 2.25 0 0 1 19.13 13.57L14.01 18.83A0.9 0.9 0 0 1 12.47 18.2L12.47 5.8Z" />
    ),
  },
  modeSequence: {
    nodes: (
