...
describe("Keyboard path (task 12.2)", () => {
  ...
  it("the song action menu is keyboard-reachable with roving focus", async () => {
    ...
    const menu = await screen.findByRole("menu", { name: "歌曲操作" });
    // First menuitem receives focus on open (task 12.1).
    ...
    expect(screen.queryByRole("menu", { name: "歌曲操作" })).not.toBeInTheDocument();
  });
});
